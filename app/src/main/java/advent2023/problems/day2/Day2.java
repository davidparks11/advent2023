package advent2023.problems.day2;
...
public class Day2 extends Puzzle {
    ...
    public Object part1() {
        int validGames = 0;
        for (String gameData : input) {
            validGames += gameUnderMaxCubes(12, 13, 14, gameData);
        }
        return validGames;
    }
    ...
    public Object part2() {
        int setSum = 0;
        for (String gameData : input) {
            setSum += powerOfMinimumCubes(gameData);
        }
        return setSum;
    }

    private int gameUnderMaxCubes(int redMax, int greenMax, int blueMax, String gameData) {
        String[] parts = gameData.split(" ");
        int gameId = Integer.parseInt(parts[1].replace(":", ""));

        int currentCubes, red, blue, green;
        currentCubes = red = blue = green = 0;

        for (int i = 2; i < parts.length; i++) {
            String part = parts[i];

            // Capture current draw value and continue
            if (i % 2 == 0) {
                currentCubes = Integer.parseInt(part);
                continue;
            }

            // Parse color and add currentCubes to color
            String color = i == parts.length - 1 ? part : part.substring(0, part.length() - 1);
            switch (color) {
                case RED -> red += currentCubes;
                case GREEN -> green += currentCubes;
                case BLUE -> blue += currentCubes;
                default -> throw new RuntimeException(
                        String.format("invalid cube color %s on game %d part %d", color, gameId, i));
            }

            // Check if end of round/end of game, return if a draw breaches max or reset
            // counts
            if (part.charAt(part.length() - 1) == ';' || i == parts.length - 1) {
    ...
    // Similar to part1, but instead we check to see if the current draw count
    // of cube is greater than the max recorder draw, as the will dictate the
    // lowest number of cubes possible for this game.
    private int powerOfMinimumCubes(String gameData) {
        String[] parts = gameData.split(" ");
        int gameId = Integer.parseInt(parts[1].replace(":", ""));

        int currentCubes, red, blue, green, maxRed, maxGreen, maxBlue;
        currentCubes = red = blue = green = maxRed = maxGreen = maxBlue = 0;
